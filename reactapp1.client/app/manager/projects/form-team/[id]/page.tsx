...
export default function FormTeam({ params }: FormTeamProps) {
  ...
  const addDeveloperToTeam = (developer: Developer) => {
    if (selectedTeam.includes(developer.name)) return
  ...
  const scheduleMeeting = () => {
    ...
    router.push(`/manager/projects/assign-tasks/${projectId}`)
  }
  ...
  const matchingDevelopers = availableDevelopers.filter((dev) =>
      ...
      {project.technologies.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center justify-center p-6">
            ...
            <Link href={`/manager/projects/add-technologies/${project.id}`}>
